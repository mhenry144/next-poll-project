...
function Post({ id, post, postPage }) {
  ...
  useEffect(
    () =>
      // go into db, create snapshot object
      onSnapshot(collection(db, 'posts', id, 'likes'), (snapshot) =>
        setLikes(snapshot.docs)
      ),
    // any variable outside of useEffect needs to be in dependency
    [db, id]
  )
  ...
  useEffect(
    () =>
      setLiked(
        // check to see if particular like (id) matches with session.user
        likes.findIndex((like) => like.id === session?.user?.uid) !== -1
      ),
    // reload when likes changes
    [likes]
  )

  const likePost = async () => {
    if (liked) {
      // once liked -> delete like document
      await deleteDoc(doc(db, 'posts', id, 'likes', session.user.uid))
    } else {
      // create collection of likes and store them in session.uesr.uid
        ...
        >
          ...
          >
            <div className="icon group-hover:bg-pink-600/10">
              {liked ? (
                // if liked show solid heart instead
